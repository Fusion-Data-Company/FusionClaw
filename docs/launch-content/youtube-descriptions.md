...
Two videos at launch. These descriptions are SEO-optimized (keywords in first 150 chars where the YouTube algorithm reads them most heavily).
...
**Tags:** open source, self hosted, AI agent, MCP, business OS, NextJS, install tutorial, fusionclaw
...
0:40 — npm run dev — dashboard renders
0:50 — Connect your AI agent via the MCP API key
0:58 — All hustle. No luck. One database.
...
**Tags:** demo, walkthrough, open source, business software, AI agent, Claude Code, MCP, fusionclaw
...
3:30 — Finance — Invoices, Expenses, Financials
4:15 — Content & Marketing — Studio, Gallery, Campaigns, AI Queue, Publishing
5:00 — Wiki Brain — file tree + force-directed graph view (Karpathy LLM Wiki pattern)
6:30 — MCP server — connecting Claude Code with one API key
7:30 — Self-hosting — three install paths (Local, Docker, Vercel)
8:30 — White-labeling for agencies
9:00 — What's next — community, roadmap, contributing

🛠️ Stack: Next.js 16 App Router, Drizzle ORM, Neon PostgreSQL, Tailwind CSS v4, Radix UI, TanStack Table + Virtual, @hello-pangea/dnd, Recharts, Framer Motion, Model Context Protocol SDK, OpenRouter, fal.ai, Resend, Vercel Blob, react-force-graph-2d.
...
- Use the same thumbnail aesthetic across both videos — mascot art + bold typography. YouTube viewers click thumbnails before titles.
- The chapters in video 2 give YouTube's algorithm structured signal. They also let viewers jump to the part they care about.
- After upload: go to Studio → Details → tag aggressively. Add "Karpathy" and "wiki" as tags too — niche search traffic.
- Pin the repo URL as the top comment under each video.
- Embed the 60-second video at the top of the README replacing the placeholder once it's recorded.
